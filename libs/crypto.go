...
func DecryptLoginPktSM2(data []byte, privkdata []byte, userCertData []byte) ([]byte, error) {
	...
	if err != nil {
		//Info.Println("read private key failed, err", err)
	...
	//Info.Println("enc_length = ", enc_length)
	r_encpac := data[offset : offset+int(enc_length)]
	offset += int(enc_length)

	r_sign := data[offset:]
	//Info.Println("r_signature ", common.ToHex(r_sign))

	sign_data := data[:offset]

	if common.SM2CertVerifySignature(user_cert, sign_data, r_sign) {
		Info.Println("Verify response signature succeed")
	} else {
		Info.Println("Verify response signature failed")
		return nil, ErrVerifySignature
	}
	//Info.Println("got signature 0x", hex.EncodeToString(r_sign))
	//Info.Println("got encrypt data length = ", len(r_encpac))
	//Info.Println("got encrypt data:0x", hex.EncodeToString(r_encpac))
	//Info.Println("decrypt with privk: 0x", privk.D.Text(16))

	dec_data, err := common.SM2PrivDecrypt(privk, r_encpac)
	if err != nil {
		Info.Println("sm2 privk decrypt failed, err", err)
		return nil, ErrSM2PrikDecrypt
	}
	Info.Println("got decrypt data:", string(dec_data))
	return dec_data, nil
}
